...
def getUrls():
    url='http://hrss.nx.gov.cn/nxsearch/search.html?code=17c79427c96&tenantId=30&searchWord=%E6%AF%95%E4%B8%9A%E7%94%9F%E5%B0%B1%E4%B8%9A'
    driver.get(url)
    ...
    while True:
        href_elements = safe_get_elements(driver, './/p[@class="nxrq_search_conmon_tit"]/a')
        ...
        has_next = safe_click(driver, './/a[@class="pagenext"]')
        if has_next:
            continue
        else:
            break

    with open('../Policy/txt/NingXiaPolicy.txt', 'w') as f:
...
# 初始化csv
def initCsv():
    with open('../Policy/csv/宁夏.csv', 'a', encoding='utf_8_sig', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["时间", "网站", "内容", "来源","浏览次数"])


def getHtml():
    with open('../Policy/txt/NingXiaPolicy.txt', 'r') as f:
    ...
    for url in hrefs:
        try:
            ...
            continue
        # 获取内容
        text_parent = safe_get_element_by_xpath(driver, './/div[@class="content"]')
        if not text_parent:
            texts = ""
        else:
        ...
        # 获取时间
        time_element = safe_get_element_by_xpath(driver, './/div[@class="content_jieshi_l"]/span[1]')
        if not time_element:
            pub_time = ""
        else:
        ...
        source_element=safe_get_element_by_xpath(driver,'.//div[@class="content_jieshi_l"]/span[2]')
        if source_element:
        ...
        #点击次数
        reader_element=safe_get_element_by_xpath(driver,'.//div[@class="content_jieshi_l"]/span[3]')
        if reader_element:
            try:
                readers=reader_element.text
                readers=str(readers).split("：")[1]
            except Exception:
                readers=0
        else:
            readers=0
