...
def getUrls():
    # 政策
    url = 'http://hrss.shandong.gov.cn/channels/ch05821/?content=5aSn5a2m55Sf5bCx5Lia'
    driver.get(url)
    ...
    # 政府信息公开
    safe_click(driver, './/a[@data-channel="政府信息公开"]')
    while True:
        href_elements = safe_get_elements(driver, './/div[@align="left"]/a')
        ...
        time.sleep(2)
        has_next = safe_click(driver, './/a[@class="laypage_next"]')
        if has_next:
            continue
        else:
            break
    ...
    # 政策法规
    safe_click(driver, './/a[@data-channel="政策法规"]')
    while True:
        href_elements = safe_get_elements(driver, './/div[@align="left"]/a')
        for ele in href_elements:
            print(ele.get_attribute('href'))
    ...
    with open('../Policy/txt/ShanDongPolicy.txt', 'w') as f:
...
# 初始化csv
def initCsv():
    with open('../Policy/csv/山东.csv', 'a', encoding='utf_8_sig', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["时间", "网站", "内容", "来源", ])


def getHtml():
    with open('../Policy/txt/ShanDongPolicy.txt', 'r') as f:
    ...
    for url in hrefs:
        ...
        # 获取时间
        time_element = safe_get_element_by_xpath(driver, './/span[@bfdi="61"]')
        if not time_element:
            pub_time = ""
        else:
        ...
        if pub_time == "":
            time_element = safe_get_element_by_xpath(driver, './/span[@bfdi="74"]')
            if not time_element:
                pub_time = ""
            else:
                pub_time = time_element.text
        ...
        # 获取内容
        text_parent = safe_get_element_by_xpath(driver, './/div[@class="show"]')
        if not text_parent:
            texts = ""
        else:
        ...
        if texts == "":
            text_parent = safe_get_element_by_xpath(driver, './/div[@id="art_content"]')
            if not text_parent:
                texts = ""
            else:
                texts_element = text_parent.find_elements_by_xpath('.//*')
